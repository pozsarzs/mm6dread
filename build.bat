@echo off
rem  MM6DRead v0.2 * Status reader program for MM6D device
...
echo.
%PPC% %FPFLAG1% %FPFLAG2% %FPFLAG3% %FPFLAG4% mm6dread.lpr
echo.
...
if not exist "source\lib\%ARCH%-%OS%\mm6dread.exe" ( echo Error: firstly run "build.bat" to compile source code! & goto end )
...
config\mkshortcut.vbs /target:"%INSTDIR%\%NAME%.exe" /shortcut:"%USERPROFILE%\Desktop\MM6DRead"
echo.
...
rd /s "%INSTDIR%"
del /q "%USERPROFILE%\Desktop\MM6DRead.lnk"
goto end
